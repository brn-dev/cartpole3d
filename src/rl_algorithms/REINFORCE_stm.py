...
class REINFORCEwSTM:
    ...
    def optimize_using_episode(self):
        ...
        reinforce_objective = self.reinforce_objective_weight * torch.cat(reinforce_objectives).mean()

        state_transition_objective = self.state_transition_objective_weight * F.mse_loss(
            torch.stack(self.episode_state_preds),
            torch.stack(self.episode_state_targets)
        )

        combined_loss = reinforce_objective + state_transition_objective
